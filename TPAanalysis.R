#......................          README       ..............................####
# Title: TPA Proteomic Data Analysis with Feature Selection and Visualization
# Author: Julia García Currás
# Description: This script loads an TPA proteomic data matrix, performes a small
#             preprocessing, including the mean between pairs of replicates for 
#             each protein, and adjusts the data structure. Then, five different 
#             feature selection (FS) algorithms are applied to identify the top  
#             20 most relevant proteins. The script also includes code to visualize   
#             the combined results of these algorithms using a Venn diagram 
#             and a clustered heatmap. Individual predictors and classfication 
#             metrics for the most shared proteins can be found at the end of the script. 
# Input: "in/Data All Proteins.txt" (MaxQuant output)
# Output: "out/featureSelectionResults_TPA.xlsx", "out/vennDiagram_TPA.svg", "out/heatmap_TPA.svg"
...
## TPA data ####
  # Open data
data0 <- read.table(file = "in/Data TPA All Proteins.txt", header = T, sep = "\t")
data0 <- as.data.frame(data0)
  # Selecting interesting columns
data <- data0 %>% 
  dplyr::select(TPA_Ctrl_U57.1:TPA_Ctrl_U71.2, 
                TPA_HF.pEF_U10.1:TPA_HF.pEF_U9.2, 
                TPA_HF.rEF_U30.1:TPA_HF.rEF_U53.2,
                Gene.names) %>%
...
dataLog[is.infinite(dataLog)] <- NA
...
replicates <- sub("^[^.]*\\.", "", individuos)
individuos <- unname(sapply(individuos, substr, start = 1, stop = 3))
...
ancho <- 0.5
downShift <- 1.8
s = sd(dataLog, na.rm = T)*ancho
m = mean(dataLog, na.rm = T) - downShift*sd(dataLog, na.rm = T)
set.seed(1234)
dfImputadoReplicates <- as.data.frame(dataLog)
for (i in 1:nrow(dataLog)){
  for (j in 1:ncol(dataLog)){
    dfImputadoReplicates[i, j] <- ifelse(is.na(dataLog[i,j]), rnorm(n = 1, mean =m, s = s), dataLog[i,j])
  }
}
...
removeDiscordancesReplicates <- function(dfQ, nReps = 2){
  ...
  for (i in seq(1, ncol(dfQ)-1, nReps)){
    ...
    dfAux <- dfQ[, i:posFinal]
    
    resultProt <- apply(dfAux, 1, function(fila){
      if (any(is.na(fila))){
        return(NA)
      } else {
        return(mean(fila, na.rm = T))
      }
    })
...
xlsx::write.xlsx(x = borutaResults, 
                 file = "out/featureSelectionResults_TPA.xlsx",
                 sheetName = "Boruta", 
...
seleccion <- mRMR.classic(data = dd, target_indices = c(1), feature_count = ncol(dfFS))
# Selecting relevant information for combining results
...
# Save graph
svg("out/vennDiagram_TPA.svg", width = 14, height = 14)
grid::grid.draw(vdia)
dev.off()
...
df_long$Protein <- factor(df_long$Protein, levels = ordered_proteins[4:1])
...
svg("out/heatmap_TPA.svg", width = 12, height = 14)
print(p1)
dev.off()
...
#...........................................................................####
# 7. Save data used in the algorithms in xlsx ####
